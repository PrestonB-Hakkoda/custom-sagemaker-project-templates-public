...
def extend_config(args, model_package_arn, pipeline_arn, stage_config):
    ...
    # ---> supplying the pipeline definition as the body causes parameter length issues
    # hackey workaround for now - maybe add as a tag instead?
    new_params = {
        ...
        # the max length is 4000
        "PipelineDefinitionBody": pipeline_definition[0:4000],
        "PipelineDefinitionBodyExtras": pipeline_definition[4000:]
    }
    new_tags = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--default-bucket", type=str, default="sagemaker-us-east-2-644944822023")
    ...
    # upload any reqs:
    run_pipeline.upload_reqs(args.default_bucket)
    ...
    # execute pipeline for local testing
    if local:
        pipeline_definition.start()
