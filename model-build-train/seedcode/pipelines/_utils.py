...
def upload_reqs(args):
    ...
    BASE_DIR_REQ = os.path.join(BASE_DIR, "foot_traffic/requirements")
    
    data_eng_req = os.path.join(BASE_DIR_REQ, "01_data_eng_reqs.txt")
    train_req = os.path.join(BASE_DIR_REQ, "02_training_prep_reqs.txt")
    eval_req = os.path.join(BASE_DIR_REQ, "03_eval_reqs.txt")
    baseline_req = os.path.join(BASE_DIR_REQ, "04_baseline_monitor_reqs.txt")
    create_feat_store_script = os.path.join(BASE_DIR_REQ, "create_feature_store.py")
    
    s3_base_uri = f"s3://{kwargs['default_bucket']}/foot-traffic/requirements"
    
    reqs_paths = [data_eng_req, train_req, eval_req, baseline_req, create_feat_store_script]
